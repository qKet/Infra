# 예매 오픈 알림 큐. backend(@Scheduled 스위퍼)가 publish, Lambda(open-alert-mailer)가 consume.
...
resource "aws_sqs_queue" "dlq" {
  name                      = "${var.project_name}-open-alert-dlq-${var.environment}"
  message_retention_seconds = 1209600 # 14일(SQS 최대치) — 원인 파악할 시간 넉넉히 확보
}

resource "aws_sqs_queue" "this" {
  name = "${var.project_name}-open-alert-${var.environment}"
