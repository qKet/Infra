# 이 큐에 SendMessage 권한을 줄 역할 — sender_role_name을 안 넘기면 아예 안 만듦(이 큐를
# 누가 쓰는지 몰라도 되는 경우도 있어서 선택적으로 둠). var.name(큐 용도 이름)이 이미
# 유일한 값이라 정책 이름 접미사로 그대로 씀 — 같은 role에 여러 큐 권한을 붙여도 안 겹침.
