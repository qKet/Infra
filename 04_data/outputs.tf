...
output "cancel_alert_queue_url" {
  description = "취소표 알림 SQS 큐 URL — CD 헬름 차트 configMaps에 CANCEL_ALERT_QUEUE_URL로 들어가는 값과 같은지 대조용"
  value       = module.cancel_alert_queue.queue_url
}

output "cancel_alert_mailer_function_name" {
  description = "취소표 알림 발송 Lambda 함수 이름 (CloudWatch Logs 조회 등에 씀)"
  value       = module.cancel_alert_mailer.function_name
}
